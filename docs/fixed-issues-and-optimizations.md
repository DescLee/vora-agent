...
- 现象：`manus-mini resume <session_id>` 进入 TUI 后，只能看到最后一条用户问题或最新一轮结果；继续执行新一轮后，旧历史又被覆盖。
- 根因：
...
- 修复：
  ...
  - TUI 中断时关闭工具线程池并从 Python 退出 join registry 中 detach `manus-tool` 线程，避免解释器退出阶段打印 `Exception ignored in threading.py`。
...
- 修复：
  ...
  - `Executor` 注册 threading 退出清理钩子，在 `concurrent.futures` 的 `_python_exit()` join 之前 detach `manus-tool` 线程。
...
在 `src/manus_mini/tools/search_tools.py` 中新增两个联网工具：
