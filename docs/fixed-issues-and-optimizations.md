...
本文用于记录 Vora 第一版开发过程中已经修复的问题和完成的体验优化。每条记录包含现象、根因、处理方式和验证方式，方便后续复盘、面试讲解和继续迭代。
...
- 优化：TUI 状态区增加 `当前上下文` 占比展示，基于当前 session 消息估算 token 使用比例。
- 验证：测试覆盖状态栏显示上下文使用比例。
...
- 处理过程：一度把 active task 过程纳入状态栏估算，但后来发现这会把“当前会话上下文”和“最近一次 LLM 请求上下文”混在一起，语义不清。
- 当前修复：状态栏拆成两个口径：
  - `当前上下文 xx.x%`：只按当前 `session.messages` 估算，用于判断当前会话历史规模。
...
- 修复：
  - 不再把 `usage.prompt_tokens` 直接覆盖为 `当前上下文`，避免状态栏随 planner/react/final 任意一次 LLM 请求跳变。
  - `usage.prompt_tokens` 改为单独展示为 `最新请求`。
  - `当前上下文` 继续按 session 消息估算，`最新请求` 按模型上下文长度计算。
- 验证：测试覆盖 `当前上下文` 与 `最新请求` 的双口径展示。
...
- 现象：在其他项目目录启动 `vora` 时，如果未命中 LLM 配置，用户只能在发送消息后看到运行失败；如果命中配置，欢迎页也看不到当前使用的模型。
- 修复：
  - 欢迎页新增“模型配置”区域。
  - 找到可用配置时展示当前模型和配置来源路径，例如 `当前模型：deepseek-v4-flash`、`配置来源：/path/to/.env`。
  - 未找到可用配置时提示配置环境变量，或在当前目录 `.env`、`~/.vora/.env`、vora 安装源码根目录 `.env` 中补齐 `LLM_PROVIDER / LLM_BASE_URL / LLM_API_KEY / LLM_MODEL`。
- 验证：`tests/test_prompt_tui.py` 覆盖 formatter 和 TUI 初始化两层展示逻辑。

### 2.20 状态栏快捷键提示占用空间

- 现象：状态栏长期展示 `Enter 发送消息 | Shift+Enter 换行`，在新增上下文占比后信息过长，影响查看真正的运行状态。
- 修复：状态栏移除发送/换行快捷键提示，只保留就绪/运行状态、`当前上下文` 和 `最新请求`。
- 当前展示：
  - 空闲：`就绪`
  - 运行：`状态 正在执行 | 当前上下文 0.0% | 最新请求 --`
- 验证：`tests/test_prompt_tui.py` 覆盖状态栏不再出现 `Enter 发送` 和 `Shift+Enter 换行`。

### 2.21 resume 后只展示最后一条历史

- 现象：`vora resume <session_id>` 进入 TUI 后，只能看到最后一条用户问题或最新一轮结果；继续执行新一轮后，旧历史又被覆盖。
- 根因：
  - resume 初始化时只把格式化后的文本写入 `output.text`，没有同步写入 `completed_transcript_blocks`。
  - `format_transcript()` 只展示最后一条用户问题，不适合直接作为 resume 的完整历史视图。
  - 新一轮完成后 `format_history()` 只拼接 `completed_transcript_blocks`，所以旧历史被丢弃。
- 修复：
  - resume 初始化时构造“历史消息”块，展示 `session.messages` 中的多轮用户、Agent 和工具摘要。
  - 如果 session 有 `active_task`，再追加当前执行块。
  - 将 resume 历史块预填进 `completed_transcript_blocks`，后续新一轮完成时不会覆盖旧历史。
- 验证：`tests/test_prompt_tui.py` 覆盖 resume 初始输出展示完整历史，以及新一轮完成后仍保留旧历史。
...
### 6.1.1 写文件确认后不应重新等待 LLM 返回同一工具调用

- 现象：用户点击写文件确认后，旧流程会用原目标重新进入 Runtime，等待下一次 LLM 再返回同一个写工具调用，文件写入不是“确认后立即执行”。
- 风险：
  - 如果下一次 LLM 不再返回同一个写工具，确认后的写入不会发生。
  - 用户已经确认的 tool call 结果没有稳定回填，容易造成工具调用链路不清晰。
- 修复：
  - `SessionManager.accept_pending_confirmation()` 直接读取 `pending_confirmation` 中保存的 `tool_name`、`tool_args` 和 `tool_call_id`。
  - 确认后立即执行原 pending 工具，并把真实写入结果作为同一个 `tool_call_id` 的 tool message 回填。
  - 原来的“需要确认”占位 tool message 会被真实结果替换，避免同一个 `tool_call_id` 出现重复 tool message。
  - 写入成功后再继续当前 active task 的后续流程，让 LLM 基于真实工具结果继续测试、总结或收口。
- 验证：`tests/test_runtime.py` 覆盖“第二次 LLM 不再返回 write_file 时，确认仍会执行已保存的 pending 写工具并写入文件”。
...
- 现象：Agent 分析项目时，`list_files` 会把 `outputs/`、`runs/`、`build/`、`.vora/` 等运行产物也列入结果，导致文件列表过大、上下文膨胀，并增加 LLM 请求失败概率。
...
- 修复：扩充默认噪声目录，覆盖常见生态：
  ...
  - IDE/本地运行产物：`.idea`、`.vscode`、`runs`、`outputs`、`.vora`。
...
- 修复：明确拆分两类口径：
  - `当前上下文`：按当前 session 消息估算，和压缩触发判断保持同类语义。
  - `最新请求`：按 LLM 返回的 `usage.prompt_tokens` 展示最近一次真实请求占比。
- 行为：用户既能看当前会话历史规模，也能看最近一次模型请求是否过大，不再用一个百分比混合表达两件事。
- 验证：测试覆盖 `当前上下文`、`最新请求` 分别计算和展示。
...
- 行为：
  ...
  - 会话文件位于 `~/.vora/projects/<project_key>/sessions/<session_id>.json`。
  - 不同项目按项目绝对路径生成不同 `project_key`，避免多个项目共用同一份 session。
  - `vora list --cwd <目录>` 列出当前工作目录下已有会话。
  - `vora resume <session_id> --cwd <目录>` 恢复上次上下文并进入 TUI。
  - 加载历史 session 时会自动补齐中断造成的不完整 tool exchange，避免旧会话 resume 后触发 tool message 完整性错误。
- 验证：测试覆盖 session 保存、加载、列表展示、resume 入口加载历史消息，以及旧 session 中断后的 tool message 自动修复。
...
- 覆盖内容：`/help`、`/save-context`、`/compact`、`忘记 <关键词>`、`确认/取消`、`vora list`、`vora resume`。
...
- 现象：`vora list --cwd <目录>` 无历史时只显示 `No saved sessions.`，用户不知道它实际读取的是哪个项目隔离目录。
...
- 修复：pytest 默认设置 `VORA_DISABLE_LOGGING=1`；`EventLogger` 在 pytest 或禁用环境变量下默认不写文件，只有显式 `enabled=True` 的日志测试才写入临时目录。
...
- 现象：`vora list` 之前会直接进入 TUI，用户无法先查看已有会话再决定是否恢复。
- 修复：
  - `vora list` 作为独立子命令列出当前工作目录下的会话。
  - `vora resume <session_id>` 恢复历史会话后再进入 TUI。
  - 默认 `vora` 仍然直接进入交互模式，避免破坏原有使用习惯。
...
- 修复：
  ...
  - 对已经存在 assistant tool call、但未返回 tool result 的调用，手动补一个 `USER_CANCELLED` tool message。
  - 如果历史中已经存在孤儿 tool message，恢复时转为 system 记录，避免后续上下文校验失败。
  - TUI 中断时关闭工具线程池并从 Python 退出 join registry 中 detach `vora-tool` 线程，避免解释器退出阶段打印 `Exception ignored in threading.py`。
  - `SessionStore.load()` 会修复旧 session 中断造成的不完整 tool exchange。
- 验证：测试覆盖中断路径下状态落盘、日志更新、无孤儿 tool message、resume 自动修复历史坏 session，以及工具线程池 detach。
...
- 现象：默认 `runs/`、`outputs/`、`.vora/sessions` 和 `.vora/memory.db` 会落在当前工程目录下，容易污染用户正在分析的项目；如果简单放到 `~/.vora` 根目录，又会导致多个项目互相混用数据。
- 修复：
  - 默认事件日志目录改为 `~/.vora/projects/<project_key>/runs`。
  - 默认 run summary 也写入同一个项目隔离 runs 根目录。
  - 默认报告产物目录改为 `~/.vora/projects/<project_key>/outputs`。
  - session 文件改为 `~/.vora/projects/<project_key>/sessions`。
  - persistent memory 改为 `~/.vora/projects/<project_key>/memory.db`。
  - `project_key` 由项目目录名和项目绝对路径 hash 组成，兼顾可读性和同名项目隔离。
  - 初始化 session store 时会非破坏性迁移旧项目内 `.vora/sessions/*.json` 和 `.vora/memory.db`，只复制缺失文件，不覆盖新项目隔离目录中已有数据。
...
- 验证：测试覆盖项目隔离目录生成、session 保存路径、memory 路径、旧项目 `.vora` 非覆盖迁移、runtime 默认 run summary 目录、session 清理项目 runs，以及工作区不生成默认 runs/outputs。
...
### 9.28 历史会话中的 `RUNTIME_TIMEOUT` 导致 `vora list` 崩溃

- 现象：移除单轮运行总时长限制后，旧 session 里保存过 `RUNTIME_TIMEOUT` 错误码，`vora list` 读取历史会话时触发 Pydantic Literal 校验失败。
...
- 根因：
  - LLM 会根据文件名猜路径，但项目源码实际在 `src/vora/`。
...
- 现象：历史会话兼容曾依赖模型 Literal 临时保留旧错误码；如果未来再次出现旧错误码或字段变化，`vora list/resume` 仍可能在 Pydantic 校验前崩溃。
- 修复：
...
- 现象：`runs`、`sessions`、`memory.db` 已经迁移到 `~/.vora/projects/<project_key>/...`，但默认 `outputs/` 仍会落在当前工作目录或 pytest 的全局临时 outputs 下。
- 根因：`AgentRuntime._default_reporter_output_dir()` 仍返回 `Path("outputs")` 或 pytest 临时全局目录，没有复用项目隔离存储路径。
- 修复：
  - 新增 `project_outputs_dir(cwd)`。
  - 默认 `Reporter.output_dir` 改为 `~/.vora/projects/<project_key>/outputs`。
...
- 现象：TUI 欢迎页只展示当前模型，不展示配置来自环境变量、当前项目 `.env`、用户 `~/.vora/.env` 还是源码根目录 `.env`。
...
- 修复：
  - 默认日志目录改为 `~/.vora/projects/<project_key>/logs`。
...
- 现象：自动压缩和手动 `/compact` 只提示“已压缩”，用户无法判断压缩前规模、目标预算和保留量。
- 修复：
  - 自动压缩提示增加压缩前估算 token、目标预算、压缩后估算 token、保留消息数和摘要 ID。
  - 手动压缩提示同样增加预算和保留信息，方便用户判断压缩是否有效。
- 验证：`tests/test_demos.py` 覆盖自动压缩提示；`tests/test_memory.py` 覆盖手动压缩提示。

### 9.45 联网搜索工具 warning 不应污染 TUI

- 现象：`web_search` 调用 `duckduckgo_search.DDGS()` 时会抛出包改名 `RuntimeWarning`；底层库在部分环境还会直接向 stderr 打印证书提示，导致这些 warning 出现在 TUI 输入区/输出区，体验很差。
- 修复：
  - `WebSearchTool.run()` 使用 `warnings.catch_warnings(record=True)` 捕获 DDGS 调用期间的 RuntimeWarning。
  - 使用 `redirect_stderr` 捕获底层库直接写到 stderr 的证书提示。
  - 捕获内容不再展示到 TUI，而是写入 `ToolResult.data["warnings"]` 和 `ToolResult.data["stderr"]`。
  - 现有 `node.jsonl` 会记录完整 tool result，因此这些信息仍可在节点日志中排查。
- 验证：`tests/test_tools.py` 覆盖 warning/stderr 不外泄，且被写入 tool result data。

### 9.46 Ctrl+C 中断时补齐工具结果并更新状态

- 现象：正在运行时按 `Ctrl+C` 退出，容易出现两个问题：
  - Python 退出阶段在线程池 join 时再次被中断，打印 `Exception ignored in: <module 'threading'> ... KeyboardInterrupt`。
  - session 中可能已保存 assistant tool call，但对应 tool result 还没追加，导致下一次 resume 出现孤儿或缺失 tool message。
- 修复：
  - `complete_interrupted_tool_messages()` 扫描 `session.messages`，为缺失的 tool result 补合成结果：`error_code: USER_CANCELLED`。
  - 已经孤立的 tool message 转为 system 记录，不再作为 OpenAI tool message 参与上下文。
  - `PromptTui.handle_interrupted_execution()` 统一处理中断：标记任务 `failed`、保存 session、更新状态栏、关闭并 detach 工具线程池。
  - `Executor` 注册 threading 退出清理钩子，在 `concurrent.futures` 的 `_python_exit()` join 之前 detach `vora-tool` 线程。
- 验证：`tests/test_context.py`、`tests/test_session.py`、`tests/test_prompt_tui.py`、`tests/test_runtime.py` 覆盖补齐 tool result、orphan 转换、状态更新、resume 修复和线程池退出清理。
...
from vora.session import SessionState
...
第一版产品设计文档明确将"复杂联网搜索"列为暂不完成项，Research Pack 仅能读取本地文件，无法获取互联网信息。"资料调研"这一核心能力在实际演示中名不副实。

### 13.2 新增工具

在 `src/vora/tools/search_tools.py` 中新增两个联网工具：

#### WebSearchTool

- 名称：`web_search`
- 搜索后端：DuckDuckGo（免费，无需 API key）
- 参数：`query`（搜索词）、`max_results`（结果数，1-20，默认 5）
- 返回：结果列表，每条含标题（title）、摘要（snippet/body）、链接（href/link）
- 风险等级：`safe`，只读，无需人工确认
...
- 名称：`fetch_webpage`
- 功能：根据 URL 获取网页纯文本内容
- 参数：`url`（完整 URL）、`max_chars`（最大字符数，1000-50000，默认 8000）
- 处理：自动剥离 HTML / head / script / style 标签及 HTML 实体
- 请求头：带 User-Agent 标识，超时 15 秒
- 返回：网页纯文本正文，超出 `max_chars` 时截断并标注 `[... content truncated ...]`
- 风险等级：`safe`，只读，无需人工确认

### 13.3 注册方式

- `tools/__init__.py`：导入并导出 `WebSearchTool`、`FetchWebpageTool`
- `tools/registry.py`：加入 `ToolRegistry` 默认工具列表，Agent 启动时自动加载
- LLM 通过 `llm.tool_schema(name)` 自动获取参数 schema，无需额外配置
...
```text
用户：调研一下 Manus 项目的最新进展
Agent：
  1. web_search(query="Manus AI Agent 2026", max_results=5)
     → 获得搜索结果列表（标题 + 摘要 + 链接）
  2. fetch_webpage(url="https://example.com/article")
     → 获取正文内容
  3. 整理调研报告
```
...
- 新增 `duckduckgo_search`：DuckDuckGo 搜索接口
- 新增 `requests`：网页内容抓取
- 两个库均已加入 `pyproject.toml` 的 `dependencies` 主依赖

### 13.6 验证

- 工具注册：`ToolRegistry` 默认包含 `web_search` 和 `fetch_webpage`
- 参数 schema：`parameters_schema()` 返回正确的 JSON Schema 定义
- 空参数检查：缺失 `query` 或 `url` 时返回 `INVALID_TOOL_PARAMS` 错误
- 非法 URL 检查：不以 `http://` 或 `https://` 开头的 URL 被拒绝
- 第三方 warning/stderr 不外泄到 TUI，统一记录到 `ToolResult.data`
- 已有测试不受影响：全量测试通过。

### 13.7 当前局限

- DuckDuckGo 在国内部分网络环境下可能不可用
- 网页抓取为纯文本提取，不支持 JS 渲染的单页应用
- 未提供搜索 API key 配置项，如需要 Google/Bing 搜索可扩展 `search_tools.py`

### 13.8 第三方搜索库 warning 处理

- 现象：`duckduckgo_search` 会提示包已改名为 `ddgs`，并在部分 macOS / keychain 环境下输出证书相关 stderr。
- 处理：
  - warning 和 stderr 都在工具内部捕获，不直接打印到终端或 TUI。
...
## 14. 面向 8 年经验面试讲解的系统补强

更新日期：2026-07-09

本轮更新针对“用于 8 年经验岗位面试讲解时，技术深度和工程可信度还不够完整”的问题进行补强。重点不是继续堆功能，而是补齐 Agent 质量门禁、评测体系、架构取舍文档、生产化说明和文档一致性。
...
## 16. Skills 与 MCP 扩展管理

更新日期：2026-07-11

### 16.1 Agent 缺少可扩展的 Skills 能力层

- 现象：
  - 旧实现只有通用 Planner/ReAct prompt 和工具注册表。
  - “项目分析”“代码修改”“面试演示”这类任务流程只能写死在 prompt 或运行时代码里。
  - 用户新增能力时需要改业务代码，不利于展示 Agent 工程里的能力分层。
- 修复：
  - 新增 `src/vora/skills/`：
    - `SkillSpec` 描述 `name/description/triggers/instructions/tool_allowlist/acceptance`。
    - loader 支持从 `skill.json` + `instructions.md` 加载项目级或用户级 Skill。
    - registry 支持内置 Skill、项目级 `./skills/*`、用户级 `~/.vora/skills/*`。
  - Runtime 在用户输入后匹配 active Skill，并写入 `task.metadata["active_skill"]`。
  - Planner prompt 注入命中的 Skill，使计划阶段受能力包约束。
  - ReAct prompt 注入命中的 Skill，并按 `tool_allowlist` 收窄传给 LLM 的工具候选。
- 当前内置 Skill：
  - `project-analysis`：项目架构、工程质量、测试体系、安全边界分析。
  - `code-change`：小步代码修改、验证和测试门禁。
  - `interview-demo`：面试讲解场景，突出 Agent 工程能力与边界。
- 验证：
  - `tests/test_skills.py` 覆盖 Skill 加载、坏目录跳过、触发词匹配、Planner/ReAct prompt 注入、工具 allowlist 和 Runtime metadata。

### 16.2 Skills 缺少 CLI 管理入口

- 现象：
  - 即使运行时支持 Skill，用户仍需要手动复制目录、删除目录和检查 `skill.json`。
  - 面试演示时缺少一个清晰的“用户如何扩展能力”的命令路径。
- 修复：
  - 新增 `src/vora/skills/manager.py`，提供 Skill 目录复制、删除和名称校验。
  - 新增 CLI：
    - `vora skills list --cwd .`
    - `vora skills add ./skills/project-analysis --cwd .`
    - `vora skills remove project-analysis --cwd .`
  - 支持 `--global`，可管理用户级 `~/.vora/skills`。
  - 内置 Skill 只展示，不允许通过 remove 删除。
- 验证：
  - `tests/test_cli.py::test_cli_skills_add_list_remove_project_skill` 覆盖项目级 Skill 的新增、展示和删除。
  - help 测试覆盖 `skills` 子命令和二级 `list/add/remove`。

### 16.3 MCP 缺少配置管理入口

- 现象：
  - 项目之前没有 MCP server 配置管理能力。
  - 无法在 CLI 中声明项目级 MCP server，也无法说明后续 MCP 工具适配层的配置边界。
- 修复：
  - 新增 `src/vora/mcp.py`：
    - `MCPServerConfig` / `MCPConfig` 配置模型。
    - 项目级配置默认写入项目隔离存储下的 `mcp.json`。
    - 用户级配置通过 `--global` 写入 `~/.vora/mcp.json`。
  - 新增 CLI：
    - `vora mcp list --cwd .`
    - `vora mcp add filesystem --command npx --arg -y --arg @modelcontextprotocol/server-filesystem --cwd .`
    - `vora mcp remove filesystem --cwd .`
  - 兼容 `--arg -y` 这类以短横线开头的 MCP 参数。
- 当前边界：
  - 本轮只完成 MCP server 配置注册和管理。
  - 尚未把 MCP server 动态转换成 Agent 可调用工具；后续应在 `ToolRegistry` / MCP adapter 层接入。
- 验证：
  - `tests/test_cli.py::test_cli_mcp_add_list_remove_project_server` 覆盖 MCP server 配置新增、展示和删除。
  - build 输出确认 `vora/mcp.py` 已进入 wheel。

### 16.4 TUI 欢迎页缺少扩展管理提示

- 现象：
  - 欢迎页只提示默认 TUI、一次性任务、历史会话和恢复会话。
  - 用户不知道 MCP 和 Skills 管理入口。
- 修复：
  - 在欢迎页“常用入口”中新增：
    - `vora mcp list --cwd .`
    - `vora skills list --cwd .`
- 验证：
  - `tests/test_prompt_tui.py::test_format_welcome_explains_limits_and_controls` 覆盖欢迎页展示 MCP 和 Skills 管理入口。
...
说明：`python -m build` 在沙箱内因 DNS/PyPI 访问失败，使用外部权限重跑后通过。
...
- 现象：
  - 项目已确定改名为 Vora，但旧包名、旧 CLI 指令、旧文档示例和测试导入路径仍混在一起。
  - 面试演示时会出现“安装名、命令名、源码目录、README 标题不一致”的问题。
- 修复：
  - Python 包从 `src/manus_mini` 迁移到 `src/vora`。
  - `pyproject.toml` 中的项目名、console script、coverage source、mypy files 统一改为 `vora`。
  - CLI 入口统一为 `vora`，包括 `vora run/resume/list/remove/clear/doctor/mcp/skills`。
  - README、测试、评测脚本和中文文档中的用户命令示例同步更新。
  - 旧 `.manus-mini` 项目本地存储仅作为迁移兼容路径保留，避免历史 session 和 memory 丢失。
- 验证：
