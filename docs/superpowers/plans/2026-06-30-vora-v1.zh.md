...
**架构：** 项目使用 Python 包结构，核心代码放在 `src/vora`。`app.py` 负责 Textual TUI，`runtime.py` 负责外层工程兜底循环，`react.py` 负责 LLM 与工具调用之间的 ReAct 循环，`reflection.py` 负责质量评估循环，`context.py` 负责上下文预算、压缩与工具消息完整性校验，`memory.py` 负责本地长期记忆，`tools/` 负责工具协议与实现。V1 默认使用 Mock LLM，确保无网络环境也能测试和演示。
...
- 创建 `src/vora/llm.py`：Mock LLM 和结构化输出模型。
...
- 创建 `src/vora/app.py`：Textual TUI 入口。
- 创建 `tests/` 下的单元测试。
...
一个用于面试展示的 TUI 版轻量本地 Agent Runtime。
