...
**Architecture:** The app is a Python package under `src/vora`. `prompt_tui.py` owns the prompt_toolkit TUI, `runtime.py` owns the outer engineering loop, `react.py` owns tool-calling ReAct iterations, `reflection.py` owns quality feedback, `context.py` owns context budgeting/compression, `memory.py` owns local memory, and `tools/` owns tool contracts plus implementations. V1 uses an explicitly configured OpenAI-compatible LLM provider so tests and demos can inject their own transport.

**Tech Stack:** Python 3.12+, prompt_toolkit, Rich, Pydantic, pytest, ruff, sqlite3, asyncio.
...
- Create `src/vora/models.py`: shared Pydantic models including `SessionState`, `TaskState`, `LoopLimits`, `Message`, `ToolCall`, `ContextSegment`.
- Create `src/vora/context.py`: token estimation, segment building, tool-call pair validation, compression/cropping.
- Create `src/vora/memory.py`: sqlite-backed memory manager.
- Create `src/vora/tools/base.py`: tool protocol, tool specs, result models.
- Create `src/vora/tools/file_tools.py`: list/read/write/append/mkdir tools with workspace path checks.
- Create `src/vora/tools/registry.py`: tool registry and default registration.
- Create `src/vora/scheduler.py`: dependency analysis and parallel batch planning.
- Create `src/vora/llm.py`: LLM client interface and structured result models.
- Create `src/vora/react.py`: ReAct tool loop.
- Create `src/vora/reflection.py`: reflection loop decisions.
- Create `src/vora/runtime.py`: outer engineering loop and failure handling.
- Create `src/vora/session.py`: session lifecycle.
- Create `src/vora/logging.py`: JSONL event logger.
- Create `src/vora/reporter.py`: final message/artifact generation.
- Create `src/vora/prompt_tui.py`: prompt_toolkit TUI entrypoint.
...
Local TUI Agent Runtime for explaining Agent engineering tradeoffs.
...
from vora.models import ContextSegment, Message, new_id
...
**Files:**
- Create: `src/vora/tools/__init__.py`
- Create: `src/vora/tools/base.py`
- Create: `src/vora/tools/file_tools.py`
- Create: `src/vora/tools/registry.py`
- Create: `src/vora/scheduler.py`
...
from vora.tools.file_tools import ListFilesTool, ReadFileTool, WriteFileTool
...
Modify `src/vora/models.py` by adding:
...
from vora.models import MemoryItem
...
### Task 6: Three-Layer Runtime with fake LLM

**Files:**
- Create: `src/vora/llm.py`
- Create: `src/vora/react.py`
- Create: `src/vora/reflection.py`
- Create: `src/vora/runtime.py`
...
from vora.models import LoopLimits, SessionState, ToolCall
...
from vora.models import Message, Observation, SessionState, TaskState
from vora.scheduler import ToolScheduler
...
from vora.models import SessionState, TaskState
...
from vora.models import LoopLimits, Message, PlanStep, SessionState, TaskState
...
**Files:**
- Create: `src/vora/session.py`
- Create: `src/vora/prompt_tui.py`
...
- [ ] **Step 4: Implement minimal prompt_toolkit TUI**
...
class VoraApp(App):
    ...
    def compose(self) -> ComposeResult:
        ...
        with Vertical():
            with Horizontal():
                yield Static("Vora ready", id="messages")
...
**Files:**
- Create: `src/vora/logging.py`
- Create: `src/vora/reporter.py`
- Modify: `src/vora/runtime.py`
...
def test_event_logger_writes_jsonl(tmp_path: Path) -> None:
    ...
    files = list((tmp_path / "logs" / "session-1").glob("*.jsonl"))
    assert len(files) == 1
...
class EventLogger:
    ...
    def record(self, run_id: str, event: dict[str, Any]) -> None:
        ...
        with (run_dir / "node.jsonl").open("a", encoding="utf-8") as file:
