...
**Architecture:** The app is a Python package under `src/vora`. `app.py` owns the Textual TUI, `runtime.py` owns the outer engineering loop, `react.py` owns tool-calling ReAct iterations, `reflection.py` owns quality feedback, `context.py` owns context budgeting/compression, `memory.py` owns local memory, and `tools/` owns tool contracts plus implementations. V1 uses an explicitly configured LLM provider so tests and demos can inject their own transport.
...
- Create `src/vora/app.py`: Textual TUI entrypoint.
...
name = "vora"
version = "0.1.0"
description = "A TUI lightweight local Agent Runtime for interview demos"
...
TUI lightweight local Agent Runtime for interview demos.
...
```bash
git add src/vora/session.py src/vora/app.py tests/test_session.py
