...
第一版目标是实现一个可运行的本地 Agent Runtime，支持一次性运行和恢复历史会话，并围绕同一个会话迭代完成三类任务：资料调研、本地项目助手、任务自动化。系统需要体现现代 Agent 工程设计，而不是简单的单轮问答。
...
- `prompt_toolkit`：构建 resume 交互恢复界面、消息区、输入区、状态栏、滚动输出和确认面板。
...
负责命令解析、一次性运行、历史会话恢复，以及恢复后的终端页面渲染、用户输入、消息列表、产物预览、状态栏和确认弹窗。
...
负责为 Planner 和项目相关 ReAct 请求生成轻量 Repo Map，只包含项目类型、根 manifest、顶层目录和固定入口候选。构建过程不递归遍历全仓、不读取源码正文，也不预提取声明符号；缓存基于这些根级元数据生成，避免每轮问答预扫描全部代码。

项目文件检索优先级为 `search_code`、`glob`、定向 `read_file`。`search_code` 支持一次提交最多 3 个查询词，优先通过 `rg --json` 返回带查询归属、文件路径、行号和片段的相关性排序结果，并遵守工作区、`.gitignore`、敏感文件、单文件大小和结果数量限制；`rg` 不可用时回退到 Python 扫描。普通 `read_file` 对不超过 4 KB 的小文件直接内联正文，大文件继续返回 `content_ref`，要求后续使用 `query` 或行号窗口精读。研究和代码审查任务默认最多调用 2 次 `search_code`、读取 6 个不同文件，第二次搜索只允许根据首轮证据扩展一层；代码修改任务不受该读取预算限制。
...
vora/
  ...
  evals/
    run_evals.py
  ~/.vora/projects/<project_key>/   # 默认运行态存储位置
    sessions/
    logs/
    outputs/
    memory.db
...
- `max_engineering_steps=3`：外层工程兜底循环默认最多 3 轮，避免本地演示任务无限展开。
- `max_react_iterations=99`：单个计划步骤默认给足 ReAct 工具循环空间，由工具调度、确认和 Reflection 控制质量。
- `max_reflection_rounds=3`：质量反馈最多 3 轮，保留修正能力但避免反复重写。
- `max_tool_calls_per_iteration=99`：单轮 LLM 返回的工具调用数量上限。
- `max_tool_retries=3`：单个工具失败最多重试 3 次。
- `tool_retry_backoff_seconds=0.1`：工具重试退避基础间隔。
- `max_tool_timeout_seconds=None`：默认不设置统一工具超时，具体工具可单独控制。
...
```text
~/.vora/projects/<project_key>/
  memory.db
  sessions/
    <session_id>.json
```

如果用户级目录不可写，会回退到当前工作区 `.vora/`，并且会非破坏性迁移旧项目内 `.vora` 与 `.manus-mini` 的历史会话和 memory。
...
压缩日志写入项目隔离存储下的会话 JSONL 日志，例如 `logs/<session_id>/node.jsonl` 与 `pipeline.jsonl`：
