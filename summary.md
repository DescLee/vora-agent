...
**Manus Mini** 是一个面向面试展示的 **TUI（终端界面）AI Agent 项目**，版本号 `0.1.0`。它是 Manus 项目的 TUI 第一版实现，核心定位是**展示现代 Agent 工程的系统化落地能力**，而非堆叠功能。

项目使用 Python 3.12+，依赖 `prompt_toolkit`、`pydantic`、`rich`、`textual` 等库构建终端交互界面和数据模型。
...
Manus Mini 不是一次 LLM 调用，而是由三层循环组成的工程系统：

```
外层：工程兜底循环（Engineering Guard Loop）
  ├── 控制最大步数、token 预算、运行时长
  ├── 处理工具超时、重试、异常降级
  │
...
| `max_runtime_seconds` | 180 | 单轮用户消息最大运行时长 |
...
```
src/manus_mini/
    app.py               — TUI 应用入口（prompt_toolkit）
...
```bash
manus-mini              # 默认 TUI（prompt_toolkit）
manus-mini-textual      # 旧版 Textual 界面
```
...
> **Manus Mini V1 是一个小而完整的 TUI Agent 系统，它证明了 Agent 不是一次 LLM 调用，而是围绕会话、目标、工具、状态、权限和三层反馈循环搭建的工程系统。**
