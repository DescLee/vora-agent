...
**Vora** 是一个本地 AI Agent Runtime 项目，当前展示版本号为 `v20260702.1644`。它的核心定位是**把现代 Agent 工程能力落到可运行系统里**，而非堆叠功能或复刻某个现有 Agent 产品。
...
- 可用于面试或技术评审中讲解：开发者理解 Agent 应用从自然语言目标到可交付产物的完整工程链路。
...
| 参数 | 默认值 | 说明 |
|------|--------|------|
| `max_engineering_steps` | 3 | 外层工程兜底循环最大步数 |
| `max_react_iterations` | 99 | 单个步骤内 ReAct 工具循环最大次数 |
| `max_reflection_rounds` | 3 | Reflection 质量反馈循环最大次数 |
| `max_tool_retries` | 3 | 单个工具调用最大重试次数 |
...
src/vora/
    cli.py               — CLI 入口
    prompt_tui.py        — 默认 TUI 与 resume 交互恢复界面
    runtime.py           — Agent 主循环
    models.py            — 核心数据模型（SessionState, TaskState 等）
    llm.py               — OpenAI-compatible LLM 抽象
    session.py           — 会话管理器
    memory.py            — 长期记忆管理器
    context.py           — 上下文构建与压缩
    planner.py           — 计划生成器
    ...
    reporter.py          — 报告生成器
    logging.py           — 事件日志
    mcp.py               — MCP server 配置管理
    skills/              — Skills 能力包加载、匹配和管理
    tools/               — 工具协议与实现
...
- 存储位置：默认位于 `~/.vora/projects/<project_key>/memory.db`；用户级目录不可写时回退到当前工作区 `.vora/memory.db`
