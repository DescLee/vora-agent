...
class ReActLoop:
    ...
    def _system_prompt_for_task(self, task: TaskState) -> str:
        ...
        parts = [
            ...
            "整文件替换不要使用 replace_in_file；如果必须整体重写已有文件，请使用 write_file 并显式传 allow_full_rewrite=true。",
            "如果任务涉及代码修改、修复、生成或删除，开始阶段必须先准备可执行测试命令或临时测试脚本；修改后必须运行测试。",
            "写入生产代码前必须先运行一次 is_test=true 的 run_temp_script，或运行项目已有测试/类型检查命令；否则写入工具会被拒绝。",
