...
from vora.models import MemoryItem, MemoryKind, MemoryScope
from vora.redaction import contains_sensitive_text
...
class MemoryManager:
    def __init__(self, db_path: str | Path) -> None:
        ...
        self.db_path = Path(db_path) if not self._memory_only else Path(":memory:")
        if not self._memory_only:
            self.db_path.parent.mkdir(parents=True, exist_ok=True)
        self._lock = threading.RLock()
        self._connection = sqlite3.connect(
            ":memory:" if self._memory_only else self.db_path,
    ...
    def delete_matching(self, query: str) -> int:
        ...
        with self._lock:
            for row in rows:
                ...
                tags = " ".join(tag.lower() for tag in self._decode_json_list(row["tags"]))
                blob = f"{content} {tags}"
                if query.lower() in blob or any(token in blob for token in tokens):
                    self._connection.execute("DELETE FROM memories WHERE id = ?", (row["id"],))
                    deleted += 1
            if deleted:
                self._connection.commit()
        return deleted
