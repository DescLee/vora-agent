...
def format_welcome(
    ...
    return "\n".join(
        [
            ...
            "",
            "常用入口",
            "- 默认 TUI 入口：manus-mini --cwd .",
            "- 一次性任务：manus-mini run \"总结一下当前项目\" --cwd .",
            "- 查看历史会话：manus-mini list --cwd .",
            "- 恢复会话：manus-mini resume <session_id> --cwd .",
            "- 注意：不需要额外 TUI 子命令，直接执行 `manus-mini` 就会进入当前界面。",
            "",
            "执行与安全",
            "- 会读取项目文件、调用工具、展示执行过程和最终产物。",
            "- 写入文件前会展示 diff 并等待确认；dry-run 模式只预览不落盘。",
            "- 会话、日志和产物默认保存在 ~/.manus-mini/projects/<project_key>。",
            "",
            *llm_lines,
            "",
            "运行限制",
