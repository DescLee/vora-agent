...
代码修改任务会优先暴露完整文件工具，避免被误判成纯审查任务后缺少 `replace_in_file` 等写入能力。ReAct 阶段会把最近成功写入、最近通过验证、最近失败工具和已修改文件注入提示词，帮助模型在接近收口时基于事实判断，而不是重复读取或重复调用失败工具。

定位文件时可使用 `glob` 批量匹配路径；同一阶段需要多个文件时，提示词要求一次性并行读取，减少一轮只读一个文件造成的工具轮次膨胀。`read_file` 对大文件、生成文件和重复覆盖范围有额外保护，优先返回摘要或要求用 `query/start_line/limit_lines` 精读目标窗口。
...
2. 当前项目：`./skills/*`
3. 用户全局：`~/.vora/skills/*`
4. 已安装插件：`~/.vora/plugins/*/repo/skills/*`
...
vora skills add https://github.com/owner/skill.git --global
...
Plugin 是一组可安装的能力包。当前版本会保存完整 Git 仓库，并自动加载仓库内的 `skills/*`；暂不执行 plugin hook，也不自动注册 MCP server。

```bash
vora plugins install https://github.com/owner/plugin.git
vora plugins install https://github.com/owner/plugin.git --name superpowers
vora plugins list
vora plugins remove superpowers
```

同名 plugin 再次安装会覆盖本地旧版本，相当于更新。运行时可用的 Skill 会自动包含已安装插件中的 `skill.json` 或 `SKILL.md`。
